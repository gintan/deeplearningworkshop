...
def main():
    ...
    image_datasets = {x: torchvision.datasets.CIFAR10(DATA_DIR, train=x=='train', transform=data_transforms[x], download=True) for x in ['train', 'val']}
    # Create training and validation dataloaders
    dataloaders_dict = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=BATCH_SIZE, shuffle=True, num_workers=4) for x in ['train', 'val']}
    #Get number of classes
    num_classes = len(set(image_datasets["train"].targets))

    # Detect if we have a GPU available
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print("Running training on {}".format(device))
